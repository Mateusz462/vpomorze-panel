...
if(($a=='delete') && (!empty($msg_id))){
	mysqli_query($con, "DELETE FROM `$tbl3` WHERE `id`='$msg_id'") or die("error while deleting: ".mysql_error());
}
...
$q=mysqli_query($con, "SELECT * FROM `$tbl3` ORDER BY `id` DESC LIMIT $limit") or die(mysqli_error());

$q_count=mysqli_query($con, "SELECT count(`id`) as count FROM `$tbl3` ") or die(mysqli_error());
$count_=mysqli_fetch_assoc($q_count);
...
$q_count=mysqli_query($con, "SELECT count(`id`) as count FROM `$tbl3` ") or die(mysql_error());
$count_=mysqli_fetch_assoc($q_count);
